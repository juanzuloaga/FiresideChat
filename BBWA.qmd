...
visit_detections_can71 <- merge(bcrlist_can71, visit, by="id")%>%
   subset(select = c("id", "tssr", "jday")) |> 
  rename(ordinal_day = jday, time_since_sunrise = tssr)
...
ggplot(birds_merged_can71, aes(x=id))+
  geom_histogram()+
  labs(x="id", y = "Counts (males per hectare")+
...
ggplot(visit_detections_can71, aes(x=ordinal_day))+
  geom_histogram()+
  labs(x="Ordinal day", y = "Counts")+
  theme(axis.text=element_text(size=14))+
  theme(axis.title = element_text(size = 16))
...
locations <- st_as_sf(visit, coords =  c("lon", "lat"), crs=4326)%>%
  terra::vect()%>%
  terra::project(crs(bcr_boundary))

locations_can71 <- terra::mask(locations, bcr_boundary[bcr_boundary$subUnit==71])%>%
  terra::crop(bcr_boundary[bcr_boundary$subUnit==71])

#locations_can81 <- terra::mask(locations, bcr_boundary[bcr_boundary$subUnit==81])%>%
 # terra::crop(bcr_boundary[bcr_boundary$subUnit==81])
...
#  SPECIES DATA
  ...
  # Pallete
    pal <- colorNumeric(palette = "RdYlBu",domain = locations_can71$year)
  ...
  # Adding species data
    addCircleMarkers(data=terra::project(locations_can71, "epsg:4326"),
                     fillColor = ~pal(locations_can71$year),
                     weight = 0.5,
                     radius = 2, 
                     color = ~pal(locations_can71$year),
                     group="Species_data")%>%
