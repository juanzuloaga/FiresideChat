...
visit_detections_can71 <- merge(bcrlist_can71, visit, by="id")%>%
  inner_join(birds_merged_can71) |> 
   subset(select = c("id", "tssr", "jday", "count", "lat", "lon")) |> 
  pivot_longer(jday:tssr, names_to="metric", values_to="value") |> 
...
ggplot(birds_merged_can71, aes(x=count))+
  geom_histogram()+
...
ggplot(visit_detections_can71, aes(x=value))+
  geom_histogram()+
  labs(x="", y = "Number of surveys with detections")+
